...
void golemkamikaze::MonsterPushUpdate(float _Delta)
{
...
void golemkamikaze::Start()
{
	Monster = CreateComponent<GameEngineSpriteRenderer>(130);



	Monster->CreateAnimation("golemkamikaze_jump", "golemkamikaze_jump", 0.1f, -1, -1, false);
	Monster->CreateAnimation("golemkamikaze_wake", "golemkamikaze_wake", 0.1f, -1, -1, true);
	Monster->CreateAnimation("golemkamikaze_walk", "golemkamikaze_walk", 0.1f, -1, -1, true);
	Monster->AutoSpriteSizeOn();
	Monster->SetAutoScaleRatio(2.0f);
	Monster->ChangeAnimation("golemkamikaze_walk");
	...
	{
		Range_Col = CreateComponent<GameEngineCollision>(ContentsCollisionType::Monster_Boom);
		Range_Col->Transform.SetLocalScale({ 300.0f,300.0f });
	}

	{
		Boom_Col = CreateComponent<GameEngineCollision>(ContentsCollisionType::Monster_Weapon);
		Boom_Col->Transform.SetLocalScale({ 250.0f,250.0f });
		Boom_Col->Off(); 
	}
...
void golemkamikaze::Update(float _Delta)
{
...
void golemkamikaze::AnimationCheck(const std::string_view& _AnimationName)
{
